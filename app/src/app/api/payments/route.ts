...
export async function POST(req: NextRequest) {
  ...
  if (!body.invoiceId || body.amount == null) {
    return fail('invoiceId y amount son obligatorios')
  }
  ...
  const result = await prisma.$transaction(async (tx) => {
    const payment = await tx.payment.create({
      data: {
        ...
        notes: body.notes,
        receiptNum: body.receiptNum,
      },
    })

    const newPaid = paid + amount
    const newStatus =
      newPaid + 0.01 >= Number(invoice.total) ? 'PAID' : newPaid > 0 ? 'PARTIAL' : invoice.status
  ...
  await audit(session, 'pagos', 'create', `Pago ${amount} en factura ${invoice.number}`, result.id)
