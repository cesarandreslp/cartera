...
export async function PUT(req: NextRequest, ctx: Ctx) {
  ...
  const invoice = await prisma.invoice.update({
    where: { id },
    data: {
      concept: body.concept ?? current.concept,
      subtotal,
      discount,
      mora,
      tax,
  ...
  await audit(session, 'facturas', 'update', `Factura ${invoice.number}`, invoice.id)
...
export async function DELETE(_req: NextRequest, ctx: Ctx) {
  ...
  if (current._count.payments > 0) {
    ...
    await audit(session, 'facturas', 'void', `Factura ${invoice.number}`, invoice.id)
    return ok({ ...invoice, voided: true })
  }

  await prisma.invoice.delete({ where: { id } })
  await audit(session, 'facturas', 'delete', `Factura ${current.number}`, id)
